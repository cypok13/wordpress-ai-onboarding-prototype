...
A clickable, coded prototype of an onboarding flow: someone describes what they want, an assistant asks a few short questions, a site draft is generated, they edit it, and they publish. Built in WordPress.com's real design language.
...
**This is a design prototype, not a product.** It exists to argue an interaction design, not to ship. Concretely:

- **There is no AI.** No model, no API, no network calls of any kind. Everything that looks like intelligence is keyword matching against hardcoded lists plus pre-written responses on a timer. See [How the "AI" works](#how-the-ai-works).
- **There is one hardcoded output.** Whatever you type, the generated site is always the same yoga studio with the same seven blocks.
- **Nothing persists.** No storage, no backend. Refresh and you are back at the start.
- **Many controls are deliberately inert.** They are drawn so the surface reads correctly, and they do nothing. [The full list is below](#what-is-real-and-what-is-not) — check it before filing a bug.
...
There is no router. `src/main.tsx` reads the query string once at startup.
...
| `/?view=lofi` | An earlier grayscale prototype, kept for history |
...
2. **Discovery** — a full-screen chat asks four questions, one at a time, with a visible counter. Answers that were already implied by the prompt are pre-filled and marked "from your description". Each answer is appended to a conversation thread.
3. **Generating** — a full-screen blocking state, five stages, about 6.5 seconds.
4. **Editor** — the draft opens in an editor built from the real component library. The same conversation thread continues in the right rail. Blocks are marked as AI drafts until edited.
5. **Publish** — publishing free on a `*.wordpress.com` address is the working path; the paid domain paths are stubs.
...
| File | What it is |
|---|---|
| `src/main.tsx` | The router. 27 lines. Read first. |
| `src/hifi/Flow.tsx` | The state machine for the whole demo. Every step transition is decided here. |
| `src/hifi/content.ts` | All user-facing copy, all data, and the fake-AI keyword matchers. The most important file. |
...
| File | What |
|---|---|
| `TopBar.tsx` | The top bar. Only undo, redo, Launch, the logo and the palette are wired |
| `Sidebar.tsx` | The three right-hand tabs, and the static Design panel |
...
| `CommandPalette.tsx` | The ⌘K palette, hand-assembled from design-system primitives |
...
| `src/lofi/` | An earlier grayscale prototype. Shares no code with `src/hifi/` |
...
1. **`detectGoals(prompt)`** — lowercases the prompt and tests it against five keyword lists. No default: an unmatched prompt returns nothing, which is why the first question has a separate heading for that case.
...
| Package | Role |
|---|---|
| `@wordpress/components` | The Gutenberg component library the real edit surface is built on. Supplies buttons, cards, toolbars, menus, tabs, modal, popover, snackbar, progress bar |
| `@wordpress/icons` | All standard glyphs |
| `@automattic/color-studio` | The palette's provenance. **Installed but never imported** — the hex values were copied into the tokens by hand |
...
These will cost you an afternoon if nobody tells you.

1. **Two different blues.** The tokens say the brand blue is `#3858e9`; the prototype overrides it to `#2d5af2` to match the design file, in four places. Know which one you are changing.
...
4. **CSS `@import` must come first.** Put any rule above those imports and they are silently dropped, taking every token with them.
...
7. **Toolbar internals are fragile.** The mover buttons must sit inside a real toolbar group; a plain wrapper breaks the library's roving focus and kills the dropdowns.
8. **Three packages were deliberately not used**, each documented in a code comment: the block editor package (needs a full editor store and a 143 KB stylesheet that collides), the commands package (needs a data store), and the search control inside the palette (its chrome could not be stripped without fighting the library).
...
- Otherwise **Fraunces** loads from Google Fonts as a stand-in. It is under the SIL Open Font License and is free to redistribute. It is close enough in character that layout and sizing hold.

**The hero photograph is not included** either, for the same reason. The code points at `public/hero-yoga.svg`, an illustration made for this prototype. Swap in a real photograph and the layout is unchanged.
...
Intake typing and submit. The type picker, with keyboard navigation. All four discovery questions, including multi-select, the free-text row, skip, the counter, the pre-fill notes and the animations. The generation sequence. In the editor: inline editing of the hero headline and the About body, block selection and hover, undo and redo, moving and deleting blocks, page switching, the ⌘K palette with search and keyboard navigation, sidebar tabs, and "start over". On the block toolbar: change tone, rewrite (About only), delete, move. In the chat: sending messages, the suggestion chips, the thinking state, and the document changes for tone, shortening About, and adding testimonials. On publish: going live free, the confirmation, and copy link.
...
**Canvas:** the between-blocks inserter opens, but its suggestions are decoration. Inside the rendered site, every button, form and navigation link is inert by design.
...
**Publish:** claim domain, connect a domain, and search for a different domain.
...
- The discovery composer looks like a chat input, but text typed there is recorded as the answer to the current question.
...
- **An alternate hero style variant** has a reducer and a stylesheet rule, but no control calls it.
- Several unused constants in `content.ts`, and two unused SVGs in `public/`.
...
They would not survive production, and each was a conscious trade for demo speed.
...
9. Desktop only: fixed viewport height, a fixed-width sidebar, no breakpoints.
...
12. **Only the website path is selectable.** The other four build types are disabled in this build. As a result the newsletter variant of the publish screen is reachable only through `?intent=newsletter`, and when reached it promises a subscribe form that the previewed draft does not contain.
13. **Some comments are stale.** Two comments describe an earlier architecture in which one editor shell persisted across discovery and generation. Those are now standalone screens, and the shell mounts only for the editor.
14. **One lint warning** in `main.tsx`, which degrades hot reload for that one file. Harmless.

### Not addressed at all

No tests, and no test runner installed. No error boundaries. No loading or error states outside the earlier grayscale prototype. No internationalisation, analytics or auth. Accessibility was attended to in places — roving focus, ARIA states, live regions, reduced-motion support — but has not been audited.
...
1. **Does intent survive across entry points today?** The whole proposal rests on one small intent object created at any door and carried to launch. Within a session that is trivial. Across surfaces it is the assumption to confirm before anything else.
2. **Can the AI path publish free on a subdomain?** The design treats publishing as free and charges for the upgrade — a domain, a plan. That is a pricing decision, not a design one.
3. **Is the builder's post-generation surface the same surface as the in-editor assistant?** The prototype treats them as one continuous experience. In the product they are distinct.
